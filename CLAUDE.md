...
**Не протестировано вживую** — нет `ANTHROPIC_API_KEY` в этой машине. Логика tool-executor'ов и бизнес-функций проверена напрямую (typecheck + eslint чистые, CRUD API вручную протестирован curl'ом), но сам agentic loop с реальным вызовом Claude API — нет. Проверить, когда появится ключ.
