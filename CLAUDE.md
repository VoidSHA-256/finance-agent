...
UI-текст (dashboard, чат) переведён на английский — весь сайт должен быть на английском, это осознанное решение пользователя.

**Статус: шаг 5 (MCP-сервер) завершён — весь план закрыт.** `src/mcp/server.ts` — `McpServer` из `@modelcontextprotocol/sdk` (1.29.0), транспорт `StdioServerTransport` (то, как Claude Desktop/Code спавнят локальный MCP-сервер). Регистрирует те же 6 тулов, что и agent layer, но **вызывает бизнес-логику из `src/lib/{categories,transactions,budgets}.ts` напрямую**, а не через `executor.ts` — тот заточен под формат Claude Messages API (JSON-строка на входе/выходе), а MCP SDK сам валидирует аргументы по Zod raw shape (`schema.shape`, не JSON Schema — другой формат от того, что использует Anthropic Messages API в `tools.ts`) и передаёт уже распарсенный объект в callback. Три Zod-схемы (`getTransactionsInput`, `categorizeInput`, `getBudgetStatusInput`), которые раньше были приватными в `executor.ts`, экспортированы оттуда — переиспользуются в MCP-сервере, не продублированы.

`.mcp.json` в корне репозитория — конфиг для регистрации сервера в Claude Code (`command: npx tsx src/mcp/server.ts`); секретов не содержит, `DATABASE_URL` сервер подхватывает из `.env` через `dotenv/config`, так же как `db:seed`/`agent:mock`. `pnpm mcp` — запустить сервер вручную. `pnpm mcp:test` (`src/mcp/test-client.ts`) — тестовый MCP-клиент на той же SDK, спавнит сервер как subprocess через stdio (ровно как это делает Claude Code) и реально вызывает `listTools`/`callTool` против локального Postgres — прогнано и проверено: 6 тулов зарегистрировано, `listCategories`/`getBudgetStatus` вернули настоящие данные, вызов с несуществующими UUID корректно дал `isError: true` с понятным текстом.

Важно: Next.js 16 — API/конвенции могут отличаться от того, что в моих (агента) тренировочных данных (например, `params` в динамических route handlers — Promise, а не объект, начиная с v15). Доки лежат прямо в репо: `node_modules/next/dist/docs/`. Сверяться с ними при написании нового Next.js-кода, а не полагаться на память. То же самое касается MCP SDK — молодой, быстро меняющийся протокол; перед написанием `server.ts` API (`registerTool`, формат `CallToolResult`) сверялся напрямую по `.d.ts`-файлам установленного пакета, а не по памяти.
